...
const SingleSupplier= () => {
    ...
    const supplier: Supplier = useSelector((state: RootState) => state.supplier.oneSupplier);
    const supplierIsFetching: boolean = useSelector((state: RootState) => state.supplier.oneSupplierIsFetching);
    const supplierIsError: boolean = useSelector((state: RootState) => state.supplier.oneSupplierError);
        ...
        <main>
            <Breadcrumbs
                breadcrumbs={[
                    ...
                    {label: 'Supplier Page', href: `/singleSupplier/${supplierId}`, active: true,},
            ...
            <div className="flex flex-col w-full items-center justify-between gap-y-2 md:flex-row my-3 ">
                <div className='flex w-full items-center justify-center gap-x-3 md:justify-end'>
                    <Link to={`/newSupplierTransaction/${supplierId}`}>
                        <Button className='bg-blue-600 text-white'>Transaction <PlusIcon className="h-5 ml-4"/></Button>
                    </Link>
                    <Link to={`/newSupplierPayment/${supplierId}`}>
                    <Button className='bg-blue-600 text-white'>Payment <PlusIcon className="h-5 ml-4"/></Button>
                    </Link>
                </div>
            </div>
            <div>
                ...
                {supplierIsFetching? <SupplierInfoSkeleton/> :
                 <SupplierInfo supplier={supplier} supplierId={supplierId} pathname={location.pathname}/>
                }
            </div>

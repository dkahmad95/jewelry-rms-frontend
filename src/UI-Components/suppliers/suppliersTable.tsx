...
import { getOneSupplier, getSuppliers } from "@/lib/apiCalls/supplierAPIs";
...
export default function SuppliersTable() {
  ...
  const suppliersIsLoading: boolean = useSelector(
    (state: RootState) => state.supplier.isFetching
  );
  const suppliersError: boolean = useSelector(
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 130,
      renderCell: (params) => {
        return <div className=" ">{params.value} g</div>;
      },
    },
    {
      field: "silverBalance",
    ...
    return (
      <div className="flex justify-center items-center mt-16">
        <span className="text-red-500 text-2xl">
          Error while Fetching Suppliers, please try again!
        </span>
      </div>
    );
  }
